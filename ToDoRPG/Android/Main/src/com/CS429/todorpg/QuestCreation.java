...
public class QuestCreation extends Activity {
	...
	private void FindViewByID() {
		...
		// location_spinner =
		// (Spinner)findViewById(R.id.creation_quest_location);
	...
	public Boolean validate() {
		 ...
		 * Toast.makeText(QuestCreation.this, "Please add a duration (hours).",
		 * Toast.LENGTH_SHORT).show(); validateStatus = false;
		 * 
		 * }
		 * 
		 * if(questDueMonth.length() == 0 || questDueDay.length() == 0){
		 * Toast.makeText(QuestCreation.this, "please add due date",
		 * Toast.LENGTH_SHORT).show(); validateStatus = false; }
		 */
...
//		}
		...
		if (questDuedate == null || questDuedate.isEmpty()) {
			...
			if (date.length != 3) {
				...
				validateStatus = false;
			} else if (date[0].length() < 1 || date[0].length() > 2 || date[1].length() > 2 || date[1].length() < 1 || date[2].length() != 4) {
				Toast.makeText(QuestCreation.this, "invalid due date format", Toast.LENGTH_SHORT).show();
				validateStatus = false;
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK && data != null) {
			if (requestCode == 0) {
				...
				// data.getExtras().getString("alarm"),
	...
	/**
	 * This method handles real alarm set... maybe I can use alarm intent or do
	 * alarm programatically...thinking...
	 */
	...
	class CreateQuest extends AsyncTask<String, String, Integer> {
		...
		protected Integer doInBackground(String... args) {
			...
			/**** ADDED ****/
			String quest_due_date = new String(month + "/" + day); // due date
																	// in form
																	// of
																	// "month/day"
			String quest_max_member = String.valueOf(capacity); // maximum
																// capacity
...
//			// params.add(new BasicNameValuePair("quest_duration",
			// questDuration));
			...
			//params.add(new BasicNameValuePair("quest_location", "Siebel")); // TODO
																			// quest_location));
			...
			JSONObject json = jsonParser.makeHttpRequest(Constants.url_create_quest, "POST", params);
		...
		protected void onPostExecute(int success) {
			if (success == 1) {
				...
				Toast.makeText(QuestCreation.this, "Failed To Create Quest!", Toast.LENGTH_SHORT).show();
			}
