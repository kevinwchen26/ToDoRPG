...
public class Summoner extends Character{
	...
	//Stampede
	//Low damge, applies stun. Raises INT by a base amount plus 20% damage done. 50 mana
	...
	//Double Attack
	//High dmg, both physical and magical defense used to calculate dmg. Raises STR and INT. 85 mana
	public void Skill_3(Character enemy) {
		int base = 25;
		int bonus = this.getINT()/3 + this.getSTR()/2;
		int total = base + bonus;
		int reduction = (this.getWIS() + this.getCON()) - (enemy.getCON() + enemy.getWIS());
		total += reduction;
		if(total < 0)
			total = 0;
		enemy.setHP(enemy.getHP() - total);
	...
	//All together
	//Instantly applies all status effects. Moderate damage, raises all combat stats. 100 mana
